...
function createCommit(node) {
  ...
  // console.log(`creating commit at revision ${node.rev} for branch ${node.branch}`);
...
svg.style.height = BRANCH_SPAN * branchRow + 100 + "px";
  // console.log(`creating branch at revision ${node.rev} for branch ${node.branch}`);
  ...
  const parentX = +getPositionOfParentNode(node.parents, "cx");
  const parentY = +getPositionOfParentNode(node.parents, "cy");
  config(horizontalLine, {
    "x1": lastXPos - COMMIT_R,
    "y1": lastYPos,
    "x2": parentX + COMMIT_SPAN - COMMIT_R,
    "y2": lastYPos,
    "stroke": branches[findParentBranch(node.parents)].color,
    "stroke-width": LINE_WIDTH
  })
...
function findParentBranch(parents) {
  return log.find(e => e.node === parents[0]).branch
}

function getPositionOfParentNode(parents, attr) {
  return [...document.querySelectorAll('circle')].find(e => e.getAttribute("node") === parents[0]).getAttribute(attr)
}
